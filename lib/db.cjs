...
/** 将连接串中的 sslmode=require|prefer|verify-ca 显式为 verify-full，避免 pg / pg-connection-string 的弃用告警 */
...
function getPool() {
  ...
  const url = raw ? normalizeDatabaseUrl(raw) : null
  if (!url) return null
  if (!pool) {
    pool = new Pool({
